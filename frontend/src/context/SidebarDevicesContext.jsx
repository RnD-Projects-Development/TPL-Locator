...
export function SidebarDevicesProvider({ children }) {
  ...
  const loadDefault = useCallback(async () => {
    ...
    setError("");
    try {
      const payload = await getDevicesRef.current({ page: 1, limit: DEFAULT_LIMIT });
      const list = normalizeList(payload);
      registerDevices(list);
      setDefaultDevices(list);
      setTotal(Number(payload?.total ?? list.length) || list.length);
    } catch (err) {
  ...
  }, [loadDefault, runSearch, debouncedSearch]);
