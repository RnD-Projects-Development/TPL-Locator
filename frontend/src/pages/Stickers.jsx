    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, padding: '4px 0' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 800, color: '#FFFFFF', margin: 0, letterSpacing: '-0.03em' }}>BLE Smart Stickers</h1>
            <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.36)', margin: '3px 0 0' }}>
              Cargo &amp; logistics tracking — click any to view details
            </p>
          </div>
        </div>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <input
            ...
            style={{
              ...
              color: 'rgba(255,255,255,0.70)', outline: 'none', width: 200,
            }}
          />
        </div>
        ...
        <div style={filterTabWrap}>
          {['All', 'Active', 'Offline'].map(s => (
            <button key={s} onClick={() => setStatusF(s)}
              style={{ padding: '5px 11px', borderRadius: 7, fontSize: 11, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s', ...(statusF === s ? filterTabActive : filterTabDefault) }}>
              {s}
            </button>
          ))}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14 }}>
        ...
        {stickerRows.map(s => {
          const isMissing = s.status === 'Missing' || s.status === 'Lost'
          const hAgo      = s.hoursAgo ?? 0
          const timeStr   = hAgo < 1 ? `${Math.round(hAgo * 60)}m ago` : `${hAgo.toFixed(1)}h ago`
          const timeColor = hAgo > 24 ? '#fca5a5' : hAgo > 12 ? '#fbbf24' : '#57657A'
          return (
            <div
              key={s.id}
